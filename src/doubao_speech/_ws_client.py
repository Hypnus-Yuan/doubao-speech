"""Low-level WebSocket client for Volcengine Doubao seed-tts-2.0.

Implements the bidirectional streaming TTS protocol framing, event
handling, and async audio streaming.

This module is intentionally internal (``_ws_client``); public surface
lives in :mod:`doubao_speech.api`. The API is stable enough for advanced
callers to import ``tts_stream`` directly, but no semver guarantee
attaches to any ``_``-prefixed symbol.
"""
...
from ._logging import trace_payloads_enabled
...
class VolcengineVoiceError(Exception):
    ...
    def __init__(
        self, message: str, *, code: int | None = None, payload: bytes | str | dict | None = None
    ):
...
async def _recv(ws, timeout: float) -> Message:
    ...
    if trace_payloads_enabled():
        logger.debug(
            "recv: type=%s event=%s payload=%r",
            msg.type.name,
            msg.event.name if msg.event else "-",
            msg.payload[:100] if msg.payload else b"",
        )
    else:
        logger.debug(
            "recv: type=%s event=%s payload_len=%d",
            msg.type.name,
            msg.event.name if msg.event else "-",
...
async def _send(ws, msg: Message) -> None:
    if trace_payloads_enabled():
        ...
        logger.debug(
            "send: type=%s event=%s payload_len=%d",
            msg.type.name,
            msg.event.name if msg.event else "-",
            len(msg.payload or b""),
        )
...
async def tts_stream(
    ...
    *,
    ...
    logger.info(
        "tts: connecting (session=%s, speaker=%s, format=%s)", session_id, speaker, audio_format
    )
    ...
    try:
        ws = await asyncio.wait_for(
            websockets.connect(
                ...
                ping_interval=20,
                ping_timeout=20,
                # Explicitly disable proxy auto-detection.  websockets >= 14
                # reads system proxy settings by default, which fails on hosts
                # that have a SOCKS proxy configured but no `python-socks`.
                # Users who actually need a proxy should set it explicitly.
                proxy=None,
            ),
            timeout=timeout,
        )
...
async def tts_to_file(text: str, output_path: str | Path, **kwargs) -> Path:
    """Stream TTS and atomically write every chunk to ``output_path``.

    On any error the pre-existing ``output_path`` (if any) is preserved:
    we stream into a sibling ``.tmp`` file and ``os.replace`` it into
    place only after the stream completes successfully with non-zero
    bytes received.
    ...
    VolcengineVoiceError
        If the server ends the stream without sending any audio bytes.
    """
    out = Path(output_path)
    out.parent.mkdir(parents=True, exist_ok=True)
    tmp = out.with_name(f".{out.name}.{uuid.uuid4().hex[:12]}.tmp")
    bytes_written = 0
    try:
    ...
    try:
        ws = await asyncio.wait_for(
            websockets.connect(
                ...
                ping_interval=20,
                ping_timeout=20,
                proxy=None,  # see TTS comment above; same rationale
            ),
            timeout=timeout,
        )
    except asyncio.TimeoutError as e:
                ...
                if msg.type == MsgType.FullServerResponse and isinstance(payload, dict):
                    ...
                    if isinstance(result, dict):
                        out = {
                            ...
                            or bool(msg.flag & Flags.LastNoSeq),
